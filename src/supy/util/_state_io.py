...
def extract_user_facing_state(
    ...
    user_state["initial_states"] = {
        k: v for k, v in user_state["initial_states"].items() 
...
def _extract_snow_state(df_state: pd.DataFrame, grid_id: int) -> Optional[Dict[str, Any]]:
    """Extract snow-related state if snow is enabled."""
    try:
        snow_state = {}
        for i, surf in enumerate(["paved", "bldgs", "evetr", "dectr", "grass", "bsoil", "water"]):
            snow_state[surf] = {
...
def _extract_vegetation_state(df_state: pd.DataFrame, grid_id: int) -> Optional[Dict[str, Any]]:
...
def _extract_temperature_state(df_state: pd.DataFrame, grid_id: int) -> Optional[Dict[str, Any]]:
...
def _apply_internal_state(initial_state: InitialStates, df_state: pd.DataFrame) -> None:
    ...
    internal_fields = [
        "dqndt", "dqnsdt", "dt_since_start", "tstep_prev",
        "lenday_id", "tmax_id", "tmin_id", "qn_av", "qn_s_av", 
        "snowfallcum"
    ]
    ...
    for field in internal_fields:
        if (field, "0") in df_state.columns:
            try:
                setattr(initial_state, field, float(df_state.loc[grid_id, (field, "0")]))
            except Exception:
                pass  # Field might not exist in this version


def _apply_user_state(initial_state: InitialStates, user_config: Dict[str, Any]) -> None:
...
def save_state_after_run(sim_result, output_dir: Union[str, Path], run_id: Optional[str] = None):
