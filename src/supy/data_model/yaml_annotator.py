...
class ValidationIssue:
    ...
    def __init__(
        self, path: str, param: str, message: str, fix: str, level: str = "WARNING"
    ):
        self.path = path
        self.param = param
            ...
            if matching_issues:
                ...
                for issue in matching_issues:
                    ...
                    annotated_lines.append(
                        f"{comment_indent}# [WARNING]  {issue.level}: {issue.message}"
                    )
                    ...
                    if issue.param not in line:
                        ...
                        annotated_lines.append(
                            f"{comment_indent}{issue.param}: {{value: <INSERT_VALUE>}}"
                        )

        return "\n".join(annotated_lines)

    def _find_issues_for_path(
        self, current_path: str, line: str
        ...
        for path, issues in self.issues_by_path.items():
            if path in current_path or current_path in path:
                for issue in issues:
                    ...
                    if issue.param in line or self._should_add_param_here(
                        current_path, issue
                    ):
...
"""
        ...
        while i < len(lines):
            ...
            if i in issues_by_line:
                ...
                for issue in issues_by_line[i]:
                    # Add warning comment
                    annotated_lines.append(
                        " " * indent + f"# [WARNING]  MISSING: {issue.message}"
                    )
                    ...
                    if issue.param == "bldgh":
                        ...
                        annotated_lines.append(
                            " " * indent
                            + "#   dz: {value: [0.2, 0.1, 0.1, 0.5, 1.6]}  # layer thickness (m)"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   k: {value: [1.2, 1.1, 1.1, 1.5, 1.6]}  # conductivity (W/m/K)"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   rho_cp: {value: [1.2e6, 1.1e6, 1.1e6, 1.5e6, 1.6e6]}  # heat capacity (J/m3/K)"
                        )

                    elif issue.param == "conductance":
                        annotated_lines.append(
                            " " * indent
                            + "# [TIP] ADD the following block under 'properties:':"
                        )
                        annotated_lines.append(" " * indent + "# conductance:")
                        annotated_lines.append(
                            " " * indent
                            + "#   g_max: {value: 3.5}  # maximum surface conductance"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   g_k: {value: 200.0}  # solar radiation coefficient"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   g_q_base: {value: 0.13}  # VPD coefficient base"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   g_q_shape: {value: 0.7}  # VPD coefficient shape"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   g_t: {value: 30.0}  # temperature coefficient"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   g_sm: {value: 0.05}  # soil moisture coefficient"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   kmax: {value: 1200.0}  # maximum solar radiation"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   s1: {value: 5.56}  # soil moisture threshold 1"
                        )
                        annotated_lines.append(
                            " " * indent
                            + "#   s2: {value: 0.0}  # soil moisture threshold 2"
                        )
                        ...
                        annotated_lines.append(
                            " " * indent
                            + f"# {issue.param}: {{value: <CHECK_DOCS>}}  # {issue.fix}"
                        )
                        ...
                        if issue.param == "co2pointsource":
                            annotated_lines.append(
                                " " * indent
                                + "# co2pointsource: {value: 0.0}  # point source emissions"
                            )
                        elif issue.param == "ef_umolco2perj":
                            annotated_lines.append(
                                " " * indent
                                + "# ef_umolco2perj: {value: 1.159}  # emission factor"
                            )
                        elif issue.param == "frfossilfuel_heat":
                            annotated_lines.append(
                                " " * indent
                                + "# frfossilfuel_heat: {value: 0.7}  # fossil fuel fraction for heating"
                            )
                        elif issue.param == "frfossilfuel_nonheat":
                            annotated_lines.append(
                                " " * indent
                                + "# frfossilfuel_nonheat: {value: 0.7}  # fossil fuel fraction for non-heating"
                            )
                        else:
                            annotated_lines.append(
                                " " * indent
                                + f"# {issue.param}: {{value: <CHECK_DOCS>}}"
                            )
                        ...
                        annotated_lines.append(
                            " " * indent
                            + f"# {issue.param}: {{value: <TODO>}}  # {issue.fix}"
                        )
        ...
        for i, line in enumerate(lines):
            ...
            if ":" in line and not line.strip().startswith("-"):
                ...
                if current_path:
                    ...
                    indexed_path = current_path[:-1] + [
                        f"{current_path[-1]}[{list_index}]"
                    ]
    ...
    def _organize_issues_by_line(
        self, path_to_line: Dict[str, int]
    ...
    def _find_best_insertion_line(
        self, issue: ValidationIssue, path_to_line: Dict[str, int]
    ) -> Optional[int]:
...
"""
    ...
    annotated = annotator.annotate_yaml_string(
        example_yaml, yaml.safe_load(example_yaml)
    )
