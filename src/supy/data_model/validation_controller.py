...
class ValidationController(BaseModel):
    ...
    def validate_config(self, verbose: bool = True) -> ValidationResult:
        ...
        if self.diagmethod_variable_enabled:
            if verbose:
                print("DiagMethod VARIABLE is ON: checking both RST and MOST parameters")
        ...
        if self.roughness_variable_enabled:
            ...
            result.skipped.append("Variable roughness validation (RoughnessMethod != VARIABLE)")
    ...
    def _validate_advanced_emissions_parameters(self) -> List[str]:
        ...
        for i, site in enumerate(sites):
            ...
            if pop_density is not None:
                if pop_density < 0:
                    errors.append(f"[ADV_EMIS] site[{i}].population_density={pop_density} invalid (must be ≥ 0)")
