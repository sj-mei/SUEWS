...
class ValidationIssue:
    ...
    def _generate_example(self) -> Dict[str, Any]:
        ...
        examples = {
            ...
            "thermal_layers": {
                "dz": {
                    "value": [0.2, 0.1, 0.1, 0.5, 1.6],
                    "_comment": "layer thickness (m)",
                },
                "k": {
                    "value": [1.2, 1.1, 1.1, 1.5, 1.6],
                    "_comment": "thermal conductivity (W/m/K)",
                },
                "rho_cp": {
...
"""
    ...
    def _json_to_annotated_yaml(
        self, data: Any, indent: int = 0, is_list_item: bool = False
    ) -> str:
                ...
                if key.startswith("_MISSING_"):
                    ...
                    if isinstance(example, dict) and "_comment" not in example:
                        ...
                        for sub_key, sub_value in example.items():
                            if isinstance(sub_value, dict):
                                ...
                                if "ref" in sub_value and sub_value["ref"] is not None:
                                    lines.append(
                                        f"{indent_str}#   {sub_key}: {{value: {self._format_value(value)}, ref: ...}}  # {comment}"
                                    )
                                else:
                                    lines.append(
                                        f"{indent_str}#   {sub_key}: {self._format_value(value)}  # {comment}"
                                    )
                            else:
                                lines.append(
                                    f"{indent_str}#   {sub_key}: {self._format_value(sub_value)}"
                                )
                    else:
                        # Simple parameter
                        ...
                        if (
                            ...
                            and "ref" in example
                            and example["ref"] is not None
                        ):
                            lines.append(
                                f"{indent_str}# {param}: {{value: {self._format_value(value)}, ref: ...}}  # {comment}"
                            )
                        else:
                            lines.append(
                                f"{indent_str}# {param}: {self._format_value(value)}  # {comment}"
                            )
                    ...
                    if isinstance(value, (dict, OrderedDict)):
                        if self._is_simple_dict(value):
                            # Inline simple dicts
                            lines.append(
                                f"{indent_str}{key}: {self._format_inline_dict(value)}"
                            )
                        else:
                        ...
                        if not value:
                            ...
                            lines.append(f"{indent_str}{key}:")
                            for item in value:
                                sub_yaml = self._json_to_annotated_yaml(
                                    item, indent + 1, is_list_item=True
                                )
                                lines.append(sub_yaml)
                    else:
