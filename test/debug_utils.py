                ...
                for attr_name in dir(self):
                    if not attr_name.startswith('_'):
                        ...
                        if isinstance(attr, pd.DataFrame):
                            ...
                            print(f"  Memory usage: {attr.memory_usage().sum() / 1024:.1f} KB")
            ...
            if is_ci:
                def debug_assertTrue(condition, msg=None):
                    if not condition:
                        ...
                        for var_name in ['ser_totalstore_change', 'ser_water_balance', 
                                       'ser_water_in', 'ser_water_out', 'test_dif']:
                            if var_name in local_vars:
                                ...
                                if isinstance(var_value, pd.Series):
                                    ...
                                    if 'test_dif' not in var_name:
                                        ...
                                        print(f"  Largest absolute value: {var_value[abs_max_idx]:.6f} at {abs_max_idx}")
                                else:
                                    print(f"  Value: {var_value}")
                        
                        # Calculate and show the difference
                        if 'ser_totalstore_change' in local_vars and 'ser_water_balance' in local_vars:
                            diff = (local_vars['ser_totalstore_change'] - local_vars['ser_water_balance']).abs()
        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if is_ci:
                    ...
                    # Save test data
                    artifact_file = os.path.join(artifact_dir, f"{artifact_name}_{func.__name__}.pkl")
                    ...
                    print(f"  File size: {os.path.getsize(artifact_file) / 1024:.1f} KB")
