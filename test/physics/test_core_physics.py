...
class TestPhysicalValidation(TestCase):
    ...
    def test_energy_balance_closure(self):
        ...
        if daytime_mask.any():
            ...
            print(
                f"  Mean absolute residual: {np.abs(daytime_residual).mean():.1f} W/m²"
            )
    ...
    def test_water_balance_consistency(self):
        ...
        df_water = df_out.SUEWS[["Rain", "Irr", "Evap", "RO", "State"]].assign(
            SoilStore=ser_soilstore, TotalStore=ser_soilstore + df_out.SUEWS.State
        )
        ...
        self.assertLess(
            max_rain_rate,
            200,  # mm/hr - extreme but possible
        ...
        self.assertLess(
            max_diff,
            1e-4,  # Slightly more relaxed than 1e-6 for robustness
    ...
    def test_turbulent_flux_relationships(self):
        ...
        if calm.any():
            ...
            self.assertLess(
                np.percentile(np.abs(QH_calm), 95),
                150,
                "Very large sensible heat flux during calm conditions",
            )
            self.assertLess(
                np.percentile(np.abs(QE_calm), 95),
                100,
...
class TestNumericalStability(TestCase):
    ...
    def test_zero_forcing_stability(self):
        ...
        if success:
            ...
            self.assertTrue(
                (QN < 0).all(), "Positive net radiation with zero shortwave"
            )
