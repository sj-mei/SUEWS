...
class TestSaveSuPy:
    ...
    def test_save_with_nan_values(self, sample_output):
        ...
        if 'SUEWS' in df_output.columns.get_level_values('group'):
            ...
            assert fcld_data.isna().all().all(), "Expected Fcld to be all NaN for this test"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            suews_files = [f for f in list_files if 'SUEWS' in str(f)]
            assert len(suews_files) > 0, "SUEWS file should be created despite NaN values"
    ...
    def test_save_output_groups_filter(self, sample_output):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(list_files) >= 2, "At least SUEWS and state files should be created"
    ...
    def test_resample_frequency(self, sample_output):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert '_30.txt' in suews_filename, f"Expected _30.txt in filename, got {suews_filename}"