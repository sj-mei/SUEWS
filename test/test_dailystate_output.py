...
class TestDailyStateOutput:
    ...
    def test_dailystate_no_resampling(self):
        ...
        n_days = len(pd.date_range(df_forcing_multi_day.index[0],
                                   df_forcing_multi_day.index[-1],
                                   freq='D'))
        ...
        assert all(h >= 23 for h in hours), "DailyState should only have values at end of day"
    ...
    def test_dailystate_save_output(self):
        ...
        with tempfile.TemporaryDirectory() as dir_temp:
            ...
            for ds_file in dailystate_files:
                ...
                data_cols = [c for c in df_saved.columns if c not in ['Year', 'DOY', 'Hour', 'Min', 'Dectime']]
                ...
                assert list(doy_values) == expected_doys, f"Missing days in output. Got DOYs: {list(doy_values)}, expected: {expected_doys}"
