...
class TestFortranStatePersistence(TestCase):
    ...
    def test_fortran_state_persistence(self):
        ...
        if diff > 1e-6:
            ...
            print("This indicates Fortran module variables are not being reset between calls.")
    ...
    def test_multiple_consecutive_runs(self):
        ...
        for i in range(3):
            ...
            print(f"   Cycle {i+1} QE: {cycle_qe:.6f} (diff from baseline: {diff_from_baseline:.6f})")
        ...
        if max_diff > 1e-6:
            ...
            self.fail(f"Multiple consecutive runs show pollution: max diff = {max_diff:.6f}")
        else:
    ...
    def test_minimal_reproduction(self):
        ...
        if diff > 1e-6:
            ...
            self.fail(f"Minimal reproduction shows pollution: QE difference = {diff:.6f}")
        else:
