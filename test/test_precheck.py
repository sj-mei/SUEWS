...
def test_precheck_warn_zero_sfr_params(caplog):
    logger_supy.propagate = True  # <--- PATCH temporanea solo per il test!
    ...
    with caplog.at_level(logging.INFO):
        result = precheck_warn_zero_sfr_params(copy.deepcopy(input_data))

    assert result == original, "Function should not modify input data"

    log_messages = [record.getMessage() for record in caplog.records]

    found = any(
        "As 'water' (sfr == 0)" in msg and "'alb'" in msg and "'thermal_layers.dz'" in msg and "'thermal_layers.k'" in msg
        for msg in log_messages
    )

    assert found, "Expected warning not found in logs.\nCaptured logs:\n" + "\n".join(log_messages)